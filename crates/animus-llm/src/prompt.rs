...
use animus_core::{ContentRating, Persona, persona::{Message, Role, Summary}};
...
pub fn build_prompt(
  ...
  if should_include_first_message(messages) {
    blocks.push(OllamaMessage { role: "assistant".to_string(), content: messages[0].content.clone(), });
  }
  ...
  let hitory_messages = if messages.len() > 1 {
    ...
    let start_index = if should_include_first_message(messages) { 1 } else { 0 };
...
fn nsfw_section_if_needed(rating: ContentRating) -> String {
  match rating {
      ContentRating::Nsfw => "NSFW Content Warning: This character may discuss adult content.".to_string(),
      _ => String::new(),
  }
}
...
mod tests {
  use super::*;
  use animus_core::{content_rating::ContentRating, persona::{Message, Persona, Role, Summary}};
  use uuid::Uuid;
  ...
  fn create_messages(count: usize, start_with_assistant: bool) -> Vec<Message> {
    ...
    for i in 0..count {
      let role = if (i % 2 == 0) == start_with_assistant {Role::Assistant} else {Role::User};
  ...
  fn test_build_prompt_new_conversation() {
    ...
    let messages = vec![create_test_message(Role::Assistant, "Hello, how are you today ?")];
    let result = build_prompt(&persona, &messages, None);

    assert_eq!(result.len(), 2, "Nouvelle conversation -> 2 messages (system + 1er msg)");
  ...
  fn test_build_prompt_existing_conversation() {
    ...
    assert_eq!(result.len(), 11, "Doit contenir le system + les 10 derniers messages");
